...
export class DataService {
  ...
  updateSafetyStatus(status: boolean): Observable<any> {
    const url = 'http://10.10.2.120:6968/safety';
    return this.http.post(url, { status });
  }

  updateAutopilotStatus(status: boolean): Observable<any> {
    const url = 'http://10.10.2.120:6968/autopilot';
    return this.http.post(url, { status });
  }
}